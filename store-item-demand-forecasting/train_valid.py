...
def train_valid(features: Features, target: Target):
    ...
    for year in tr_features.year.unique():
        ...
        trn_features_pure = trn_features.drop(["date", "year", "month"], axis=1)
        val_features_pure = val_features.drop(["date", "year", "month"], axis=1)
        loss = model.train_valid(trn_features_pure, trn_target, val_features_pure, val_target)
        losses.append(loss)
