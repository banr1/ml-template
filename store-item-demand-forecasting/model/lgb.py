...
from ..loss import smape, smape_grad, smape_hess


class LgbModel:
    def train_valid(self, trn_features: TrFeatures, trn_target: TrTarget, val_features: TrFeatures, val_target: TrTarget) -> float:
        trn_target_mean = int(trn_target.mean())
        trn_init_score = pd.Series([trn_target_mean for _ in range(len(trn_target))], index=trn_target.index)
        val_init_score = pd.Series([trn_target_mean for _ in range(len(val_target))], index=val_target.index)
        ...
        return model.best_score["valid"]["SMAPE"]
